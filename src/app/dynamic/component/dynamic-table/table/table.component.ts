...
export class TableComponent implements OnInit, OnChanges  {
    ...
    processCheckSelectAll() {
		...
		for (let rowIndex in this.tableCreation.data.data) {
	...
	getData(fieldData,row) {
		let dataRow = this.tableCreation.data.data[row];
		let strData = "";
    ...
    getPrimary(rowIndex) {
        ...
        let dataRow = this.tableCreation.data.data[rowIndex];
        let primaryKey = [];
    ...
    checkIgnore(rowIndex) {
		if (typeof(this.tableCreation.ignoreSelect) != "undefined") {
			...
			let dataField = this.tableCreation.data.data[rowIndex][dataSplitCheck[0]];
